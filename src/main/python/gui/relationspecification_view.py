...
class RelationSpecificationPanel(ModuleSpecificationPanel):
    ...
    def handle_handpartbutton_clicked(self):
        ...
        if self.y_h2_radio.isChecked():
            h2label = "Y"
        handpart_selector = BodypartSelectorDialog(bodyparttype=bodyparttype, bodypart1label=h1label,
                                                   bodypart2label=h2label,
                                                   bodypart1infotoload=self.bodyparts_dict[bodyparttype][1],
                                                   bodypart2infotoload=self.bodyparts_dict[bodyparttype][2], parent=self)
        handpart_selector.bodyparts_saved.connect(lambda bodypart1, bodypart2:
                                                  self.handle_bodyparts_saved(bodypart1, bodypart2, self.handpart_button))
    ...
    def handle_legpartbutton_clicked(self):
        ...
        if self.y_l1_radio.isChecked():
            ...
            l1label = "part of Y"
            l2label = "part of Y"
        legpart_selector = BodypartSelectorDialog(bodyparttype=bodyparttype, bodypart1label=l1label,
                                                  bodypart2label=l2label,
                                                  bodypart1infotoload=self.bodyparts_dict[bodyparttype][1],
                                                  bodypart2infotoload=self.bodyparts_dict[bodyparttype][2], parent=self)
        legpart_selector.bodyparts_saved.connect(lambda bodypart1, bodypart2:
                                                 self.handle_bodyparts_saved(bodypart1, bodypart2, self.legpart_button))
