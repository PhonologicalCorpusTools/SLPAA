...
class AlignTestDialog(QDialog):
    ...
    def __init__(self, app_settings, corpus, **kwargs):
        ...
        self.alignrelbycontact = QPushButton("Align relation modules based on contact")
        self.alignrelbycontact.clicked.connect(self.handle_alignrelbycontact)
        buttonoptionslayout.addWidget(self.alignrelbycontact)
        self.alignrelbyX = QPushButton("Align relation modules based on X")
        self.alignrelbyX.clicked.connect(self.handle_alignrelbyX)
        buttonoptionslayout.addWidget(self.alignrelbyX)
        self.alignrelbyY = QPushButton("Align relation modules based on Y")
        self.alignrelbyY.clicked.connect(self.handle_alignrelbyY)
        buttonoptionslayout.addWidget(self.alignrelbyY)
        self.alignrelbyexisting = QPushButton("Align relation modules based on anchor modules")
        self.alignrelbyexisting.clicked.connect(self.handle_alignrelbyexisting)
        buttonoptionslayout.addWidget(self.alignrelbyexisting)
    ...
    def handle_alignrelbycontact(self, checked):
        self.aligndisplay.setText("aligning relation modules by contact...")
        # TODO not working properly -- see Example 2A vs Example 2B, 3A vs 3B

        allalignedmodules = []

        sign1 = [s for s in self.corpus.signs if s.signlevel_information.entryid.counter == int(
            self.sign1combo.currentText()[:self.sign1combo.currentText().index(":")])][0]
        sign2 = [s for s in self.corpus.signs if s.signlevel_information.entryid.counter == int(
            self.sign2combo.currentText()[:self.sign2combo.currentText().index(":")])][0]
        alignmodel = AlignModel(sign1, sign2)

        alignedbycontact, notalignedbycontact = alignmodel.alignbyrel_contact(
            {1: list(sign1.getmoduledict(ModuleTypes.RELATION).values()),
             2: list(sign2.getmoduledict(ModuleTypes.RELATION).values())})
        alignedbycodingorder, _ = alignmodel.alignbycodingorder(notalignedbycontact, True)
        allalignedmodules.extend(alignedbycontact)
        allalignedmodules.extend(alignedbycodingorder)

        resultstring = ""
        for mod1, mod2 in allalignedmodules:
            mod1string = "no match" if mod1 is None else sign1.getmoduleabbreviation(mod1)
            mod2string = "no match" if mod2 is None else sign2.getmoduleabbreviation(mod2)
            resultstring += "S1: " + mod1string + "\n" + self.gethackymoduleabbreviation(mod1) + \
                            "S2: " + mod2string + "\n" + self.gethackymoduleabbreviation(mod2) + "\n"

        self.aligndisplay.setText(resultstring)

    def handle_alignrelbyX(self, checked):
        self.aligndisplay.setText("aligning relation modules by X...")
        ...
        alignedbyX, notalignedbyX = alignmodel.alignbyrel_x(
            {1: list(sign1.getmoduledict(ModuleTypes.RELATION).values()),
             2: list(sign2.getmoduledict(ModuleTypes.RELATION).values())},
            "contact type")
        alignedbycodingorder, _ = alignmodel.alignbycodingorder(notalignedbyX, True)
        allalignedmodules.extend(alignedbyX)
        allalignedmodules.extend(alignedbycodingorder)
    ...
    def handle_alignrelbyY(self, checked):
        self.aligndisplay.setText("aligning relation modules by Y...")
        # TODO the match list contains duplicates
        ...
        alignedbyY, notalignedbyY = alignmodel.alignbyrel_y({1: list(sign1.getmoduledict(ModuleTypes.RELATION).values()),
                                                             2: list(sign2.getmoduledict(ModuleTypes.RELATION).values())})
        alignedbycodingorder, _ = alignmodel.alignbycodingorder(notalignedbyY, True)
        allalignedmodules.extend(alignedbyY)
        allalignedmodules.extend(alignedbycodingorder)
    ...
    def handle_alignrelbyexisting(self, checked):
        self.aligndisplay.setText("aligning location modules, movement modules, and relation modules by existing anchor modules...")
        # TODO crash at relmods_withthistypeofanchor[1].pop(i) for Ex 2, 3
        ...
        alignedbylocation, locwarning = alignmodel.alignmodules(ModuleTypes.LOCATION)
        allalignedmodules.extend(alignedbylocation)
        alignedbymovement, movwarning = alignmodel.alignmodules(ModuleTypes.MOVEMENT)
        allalignedmodules.extend(alignedbymovement)
        alignedbyexisting, notalignedbyexisting = alignmodel.alignbyrel_existingmod({1: list(sign1.getmoduledict(ModuleTypes.RELATION).values()),
                                                                                     2: list(sign2.getmoduledict(ModuleTypes.RELATION).values())})
        alignedbycodingorder, _ = alignmodel.alignbycodingorder(notalignedbyexisting, True)
        allalignedmodules.extend(alignedbyexisting)
        allalignedmodules.extend(alignedbycodingorder)
