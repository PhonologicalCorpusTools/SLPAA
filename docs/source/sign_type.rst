.. todo::
  ...
  is the movement direction relation section up to date?
      - incorporate detailed notes from other sections that are better suited here
...
This :ref:`module` is used to identify the overarching ‘kind’ of sign by selecting specifications for the behaviours of each hand (H1 & H2) both separately and in relation to each other, if applicable. This module is atypical in that only one :ref:`instance` can be called for each sign, and the information coded within it applies to the sign as a whole. For more discussion on the use of modules in SLP-AA to encode information about signs, see :ref:`modularity`.
...
**(new section - add description from system overview)**
